...
impl<'f, 'tcx> Coerce<'f, 'tcx> {
        ...
        if a.is_never() {
            ...
            return if self.shallow_resolve(b).is_ty_var() {
                ...
                let diverging_ty = self.next_diverging_ty_var(TypeVariableOrigin {
                    ...
                    span_source: self.cause.span_source,
                });
    ...
    fn coerce_borrowed_pointer(
        ...
        let span_source = self.cause.span_source;
        ...
        for (referent_ty, autoderefs) in autoderef.by_ref() {
            ...
            let r = if !self.use_lub {
                ...
                if r_borrow_var.is_none() {
                    ...
                    let coercion = Coercion(span_source);
        ...
        let r_borrow = match ty.kind() {
            ...
            _ => span_bug!(span_source.to_span(self.tcx), "expected a ref type, got {:?}", ty),
        };
    ...
    fn coerce_unsized(&self, mut source: Ty<'tcx>, mut target: Ty<'tcx>) -> CoerceResult<'tcx> {
        ...
        let reborrow = match (source.kind(), target.kind()) {
            (&ty::Ref(_, ty_a, mutbl_a), &ty::Ref(_, _, mutbl_b)) => {
                ...
                let coercion = Coercion(self.cause.span_source);
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub fn try_coerce(
        ...
        let cause = self.cause(SpanSource::Span(expr.span), ObligationCauseCode::ExprAssignable);
    ...
    pub fn can_coerce(&self, expr_ty: Ty<'tcx>, target: Ty<'tcx>) -> bool {
        ...
        let cause = self.cause(SpanSource::DUMMY, ObligationCauseCode::ExprAssignable);
            ...
            Err(coercion_error) => {
                ...
                match cause.code {
                    ObligationCauseCode::ReturnNoExpression => {
                        err = struct_span_err!(
                            fcx.tcx.sess,
                            cause.span_source.to_span(fcx.tcx),
                            E0069,
    ...
    fn add_impl_trait_explanation<'a>(
        ...
        if has_impl {
            if is_object_safe {
                ...
                let sugg = vec![sp, cause.span_source.to_span(fcx.tcx)]
                    .into_iter()
                    .flat_map(|sp| {
                        vec![
