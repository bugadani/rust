...
impl<'a, 'tcx> TypeChecker<'a, 'tcx> {
    pub(super) fn equate_inputs_and_outputs(
        ...
        for (argument_index, &normalized_input_ty) in normalized_input_tys.iter().enumerate() {
            ...
            let mir_input_span = body.local_decls[local].source_info.span_source;
        ...
        if let Some(mir_yield_ty) = body.yield_ty {
            ...
            let yield_span = body.local_decls[RETURN_PLACE].source_info.span_source;
            self.equate_normalized_input_or_output(ur_yield_ty, mir_yield_ty, yield_span);
        }

        // Return types are a bit more complex. They may contain opaque `impl Trait` types.
        let mir_output_ty = body.local_decls[RETURN_PLACE].ty;
        let output_span = body.local_decls[RETURN_PLACE].source_info.span_source;
    ...
    fn equate_normalized_input_or_output(&mut self, a: Ty<'tcx>, b: Ty<'tcx>, span: SpanSource) {
