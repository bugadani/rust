...
impl<'mir, 'tcx: 'mir, M: Machine<'mir, 'tcx>> InterpCx<'mir, 'tcx, M> {
    ...
    fn binary_float_op<F: Float + Into<Scalar<M::PointerTag>>>(
        ...
        let (val, ty) = match bin_op {
            ...
            _ => span_bug!(self.cur_span().to_span(*self.tcx), "invalid float op: `{:?}`", bin_op),
        };
        (val, false, ty)
    }
        ...
        match layout.ty.kind() {
            ty::Bool => {
                ...
                let res = match un_op {
                    Not => !val,
                    _ => {
                        span_bug!(self.cur_span().to_span(*self.tcx), "Invalid bool op {:?}", un_op)
                    }
                };
